...
void addNode()
{
    ...
    // if the list is empty, make the new node the START
        ...
        // membuat nilai pref = null dan start = node baru
    ...
    else
    {
        ...
        // looping selama current != null dan noMhs dari current lebih kecil dan tidak sama dengan noMhs node baru
        while (current != NULL && current->noMhs < newNode->noMhs)
        {                               // step 1.c: traverse the list
            previous = current;         // step 1.d: move the previous to the current
            current = current->next;    // step 1.e: move to the next node
        }
        ...
        if (current != NULL)
        {
            current->prev = newNode; //step 6: make the previous fieid of the current
        }
        
        // kondisi jika previous tidak sama dengan null
        if (previous != NULL)
        {
            previous->next = newNode; // step 7: make the new field of the previous node

        }
        // kondisi jika previous  sama dengan null
        else
        {
            // if previous is still null, if means newNode is now the first node
            START = newNode; 
        }
    }
}
...
if (current == NULL)
{
    cout << "\033[31mThe record whith roll number " << rollNo << " not found\033[0m" << endl;
    return;
}
...
{ // node to be deleted is not the first node
    ...
    {  // if there's a successor, update its previous pointer
    ...
    // prosedur traverse untuk menampilkan data
    void traverse()
    {
        if (listEmpty())
           cout << "\nList is empty" << endl;
        ...
        {
            cout << "nRecords in descending order of roll number are:" << endl;
